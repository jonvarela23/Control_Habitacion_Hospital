...
#define ADC_Potenciometro_Luz_Habitacion	ADC_CHANNEL_6 // GPIO34 en WROVER-KIT
...
void adc_simple_init()
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, ADC_Potenciometro_Luz_Habitacion, &config));
    do_calibration1_chan0 = adc_simple_calibration_init(ADC_UNIT_1, ADC_Potenciometro_Luz_Habitacion, ADC_ATTEN , &adc1_cali_chan0_handle);
...
{
	int rawSample;
	ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, ADC_Potenciometro_Luz_Habitacion, &rawSample));
	return rawSample;
}
